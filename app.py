...
# --- NADOGRADNJA I INICIJALIZACIJA BAZE ---
def init_db():
    ...
    # Sigurna nadogradnja baze s novim kolonama (ako već ne postoje)
    try: c.execute("ALTER TABLE items ADD COLUMN year INTEGER DEFAULT 0")
    except: pass
    try: c.execute("ALTER TABLE items ADD COLUMN mileage INTEGER DEFAULT 0")
    except: pass
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    # Pokušaj izvući godinu i km (za aute)
                    year = 0
                    ...
                    if row:
                        ...
                        c.execute('''UPDATE items SET current_price=?, previous_price=?, price_drop=?, 
                                     description=?, last_updated=?, title=?, year=?, mileage=? WHERE ad_id=? AND filter_id=?''', 
                                  (price, old_price, new_drop, desc_text, now, title, year, mileage, ad_id, filter_id))
                    else:
                        c.execute('''INSERT INTO items (ad_id, filter_id, title, link, current_price, previous_price, 
                                     price_drop, description, last_updated, year, mileage) 
                                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                                  (ad_id, filter_id, title, link, price, price, 0, desc_text, now, year, mileage))
                    uspjesno += 1
    ...
    <div class="container">
        <div class="toolbar">
            <div class="btn-group">
                ...
                {% if filter_obj %}
                <button onclick="openEditModal('{{ filter_obj['name'] }}', '{{ filter_obj['url'] }}')" class="btn btn-blue">✏️ Uredi</button>
        ...
        {% if not items %}
            <div class="card" style="text-align:center; color:#666;">Nema podataka. Probaj Osvježiti.</div>
        {% endif %}
        ...
        <div class="card">
            ...
            <div class="ai-box">
                ...
                {% if item.get('ai_score', 0) > 0 %}
                    <span class="ai-good">Cijena je {{ item['ai_score'] }}€ niža od očekivane</span> za ovo godište i km.
                {% else %}
...
def index():
    ...
    # --- DINAMIČKI AI ALGORITAM ZA AUTE ---
    if is_car and items:
        ...
        if valid:
            ...
            for i in items:
                if i['year'] > 0:
                    ...
                    # Za svaku noviju godinu dodaje 1800€, za svaki km skida 0.06€
                    expected = avg_price + (y_diff * 1800) - (m_diff * 0.06)
                    i['ai_score'] = int(expected - i['current_price'])
        
        # Ako je korisnik odabrao "Best Buy" sortiranje, obavi to u memoriji
        if sort_by == 'best_buy':
            items.sort(key=lambda x: x.get('ai_score', -99999), reverse=True)

    return render_template_string(HTML, filters=filters, current_filter=current_filter, 
                                  filter_obj=filter_obj, items=items, is_car=is_car, sort_by=sort_by)
